...
def check_history_staleness(cache_dir: Path, last_run_file: Path):
    """Check if history is stale and clear if necessary."""
    current_time = int(time.time())

    # Check for stale data based on last run timestamp
    if last_run_file.exists():
        try:
            last_time = int(last_run_file.read_text().strip())
            ...
            if time_diff > 6:
                ...
                mem_history_file.unlink(missing_ok=True)
                swap_history_file.unlink(missing_ok=True)
        except (ValueError, IOError):
            pass

    # Update last run timestamp
    try:
        last_run_file.write_text(str(current_time) + '\n')
    except IOError:
        pass
...
def main():
    ...
    # Check for stale history and clear if necessary
    last_run_file = cache_dir / 'memory-last-run.txt'
    check_history_staleness(cache_dir, last_run_file)
    ...
    if not extended:
        ...
        print(f"row: [status:{status}] Memory: {mem['used']}MB / {mem['total']}MB")
        print(f"row: [progress:{int(mem['percent'])}:warm]")
        print("row: ")

        # Memory breakdown table
        print("row: [bold]Memory Breakdown:[/]")
        print("[table:Type|Usage]")
        print(f"[tablerow:RAM Used|[miniprogress:{int(mem['percent'])}:12:warm]]")

        if mem['swap_total'] > 0:
            print(f"[tablerow:Swap Used|[miniprogress:{int(mem['swap_percent'])}:12:cool]]")
        else:
        ...
        print(f"row: [graph:{mem_history}:warm:Memory %:0-100]")

        # Swap graph if configured
        if mem['swap_total'] > 0 and swap_history:
            print("row: ")
            print("row: [bold]Swap Usage History:[/]")
            print(f"row: [graph:{swap_history}:cool:Swap %:0-100]")
        ...
        print(f"[tablerow:Used|{mem['used']}MB|{int(mem['percent'])}%]")
        ...
        if mem['swap_total'] > 0:
            ...
            print(f"[tablerow:Swap Used|{mem['swap_used']}MB|{int(mem['swap_percent'])}%]")
